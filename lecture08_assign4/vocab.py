...
class VocabEntry(object):
    ...
    def to_input_numpy(self, sents, source=True):
        """
        ...
        sents_var = np.array(sents_t).astype(np.int32)
        if source:
            word_lengths = [len(word_id) for word_id in word_ids]
            return sents_var, np.array(word_lengths)
        else:
            word_lengths = [len(word_id)-1 for word_id in word_ids]
            return sents_var[:, :-1], np.array(word_lengths)
...
if __name__ == '__main__':
    ...
    print([i for i in vocab.src.word2id.keys()][:8])
