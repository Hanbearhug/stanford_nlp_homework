...
class ParserModel:
    ...
    def forward(self, t):
        ...
        embedding = tf.nn.embedding_lookup(self.pretrained_embeddings, t, name='embedding') ## (batch_size, n_features, embed_size)
        embedding_flatten = tf.layers.flatten(embedding)

        hidden = tf.layers.dense(embedding_flatten, self.hidden_size, activation=tf.nn.relu, name='hidden')

        dropout = tf.layers.dropout(hidden, rate=self.dropout_prob, training=self.is_train, name='dropout')
        logits = tf.layers.dense(dropout, self.n_classes, name='logits')
        ### END YOUR CODE
        return logits
